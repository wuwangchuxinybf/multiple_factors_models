...
#        self.return_month = np.load(add_ready+'wind_return_month.npy') #个股月度收益率
#        self.industry_sw1 = np.load(add_ready+'industry_sw1.npy') #申万一级行业哑变量
#        self.industry_sw1_name = np.load(add_ready+'industry_sw1_name.npy').reshape(1,-1) #申万一级行业分类名称
#        
#    def group_net_value(self):
#    
#        capital_initial = 100000000
#        factor_df = pd.DataFrame(self.factor_arr,index=stockcode,columns=trade_date)
#        netvalue_df = pd.DataFrame(index = self.trade_date,
#                                   columns=['group1','group2','group3','group4','group5'])
#        netvalue_df.iloc[0,:]=capital_initial
...
#            mid_df.rename(columns={pe.columns[n]:'pe',pe.columns[n+1]:'return_month'},inplace = True)
#            mid_df.sort_values(by='pe',ascending=False,inplace=True)
#            mid_df.dropna(inplace=True)
#            #资金等权
#            for group_num in range(5):       
#                group_amount = int(mid_df.shape[0]/5) #每组股票数量
#                if group_num==4:
#                    netvalue_df.iloc[n+1,group_num] = netvalue_df.iloc[n,group_num] + \
#                                int(netvalue_df.iloc[n,group_num]/group_amount)*\
#                               (mid_df.iloc[group_num*group_amount:,1].sum()/100)
#                else:
#                    netvalue_df.iloc[n+1,group_num] = netvalue_df.iloc[n,group_num] + \
#                               int(netvalue_df.iloc[n,group_num]/group_amount)*\
#                               (mid_df.iloc[group_num*group_amount:(group_num+1)*group_amount,1].sum()/100)
#        netvalue_df = netvalue_df/capital_initial
#        return netvalue_df                             
# 
#
#
...
#    
#    
#    mid_ic = np.corrcoef(res_ols.resid,return_month[:1256,0])[0,1]
#    
#    
#    
#    
#    # ep_traditional回归
#    res_regr_tra=pd.DataFrame(index=np.arange(ep_num.shape[1]-1),
#                                columns=['trade_month','Beta_OLS','Tvalue_OLS',\
#                                         'Beta_WLS','Tvalue_WLS','IC'])
#    for n in np.arange(ep_num.shape[1]-1):
#        #剔除空值
#        nona_index = (~np.isnan(ep_num[:,n])) & (~np.isnan(return_month[:,n+1]))
#        ep_traditional_nona = ep_num[:,n][nona_index]
